...
import { Eye, EyeOff, GithubIcon, Mail, ArrowLeft } from "lucide-react";
...
export default function LandlordRegisterPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    userType: 'landlord', // Default to landlord for this page
    phone: '',
    agreeToTerms: false,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call the register API with landlord role
      const response = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({
          ...
          role: 'landlord', // Force role to be landlord
      ...
      toast({
        title: "Success!",
        description: "Your landlord account has been created successfully.",
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary/10 to-primary/30 bg-cover bg-center py-12 px-4 sm:px-6 lg:px-8" 
      ...
      <div className="max-w-2xl w-full space-y-8 bg-white/90 dark:bg-gray-800/95 backdrop-blur-sm p-8 rounded-xl shadow-2xl">
        <div className="flex justify-between items-center">
          <Link href="/register" className="flex items-center text-primary hover:text-primary/90">
            <ArrowLeft className="h-4 w-4 mr-1" />
            <span className="text-sm">Back to Regular Registration</span>
          </Link>
        </div>
        
        <div className="text-center">
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white">
            {t?.auth?.register?.landlordTitle || 'Create a Landlord Account'}
          </h2>
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            {t?.auth?.register?.landlordSubtitle || 'Register as a property owner to list your properties'}
          </p>
        </div>
          ...
          >
            {isLoading ? (
              <span className="flex items-center justify-center">
                ...
                {t?.auth?.register?.creating || 'Creating landlord account...'}
              </span>
            ) : (
              t?.auth?.register?.createLandlord || 'Create Landlord Account'
            )}
          </Button>
